...
def get_profile_by_user_db(db: Session, user_id: str) -> Profile:
    return db.query(Profile).filter(Profile.user_id == user_id).first()

def update_profile_by_id_db(db: Session, user_id: str, profile_update: ProfileUpdate) -> Profile:
...
def delete_profile_by_user_db(db: Session, user_id: str):
    profile = db.query(Profile).filter(Profile.user_id == user_id).first()
    if not profile:
        return None
    db.delete(profile)
